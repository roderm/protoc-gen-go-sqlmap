...
func (m *Employee) Update(s *TestStore, ctx context.Context) error {
	query := squirrel.Update("tbl_employee").Where(squirrel.Eq{
		"employee_id": m.Id,
	})
	query.Set("employee_firstname", m.Firstname)
	query.Set("employee_lastname", m.Lastname)
